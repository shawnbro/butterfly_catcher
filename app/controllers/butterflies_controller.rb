class ButterfliesController < ApplicationController
  ...
  def create
    @butterfly = Butterfly.create(game_id: params[:game_id], name: params[:name], description: params[:description], pointValue: params[:pointValue], image: params[:image], caught: false)
  ...
  def update
    Butterfly.where(_id: params["butterfly"][:_id]["$oid"]).update(caught: params["caught"])
     @butterfly = Butterfly.all.uniq.sample
     update_session(Butterfly.find_by(_id: params["butterfly"][:_id]["$oid"]).pointValue)
     render json: Butterfly.where(caught: false).sample || Butterfly.create(name: @butterfly.name, description: @butterfly.description, pointValue: @butterfly.pointValue, image: @butterfly.image, caught: false)
  end
